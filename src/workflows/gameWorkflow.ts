  ...
  return {
    ...
    async execute(initialState: Day1WorkflowState): Promise<Day1WorkflowState> {
      ...
      try {
        ...
        state.elderResponse =
          'モーガン村長から重要な知らせがあります。30日後、この村に魔王が襲来します。';
        state.prophecyDelivered = true;
      }
  ...
  return {
    ...
    async execute(initialState: DayProgressionWorkflowState): Promise<{
      ...
      try {
        ...
        console.log(
          `🌅 Day ${state.currentDay}終了 -> Day ${newDay}開始 (緊張度: ${tensionLevel}%)`
        );
...
export class GameWorkflowManager {
  ...
  constructor(agents: { gameMaster: any; elderMorgan: any; merchantGrom: any; elaraSage: any }) {
    this.agents = agents;
  }
