...
export class AIVISEnhancedService {
   ...
   */
  private getModelUuid(voice: string): string {
    // 提供された正確なモデルUUID
   ...
   * 話者UUID取得
   */
  private getSpeakerUuid(voice: string): string {
    // 提供された正確な話者UUID
   ...
   * NPCごとの音声設定
   */
  async speakAsCharacter(character: string, dialogue: string, day: number): Promise<string | null> {
